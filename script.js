...
function updatePresentationPreview() {
    ...
    presentationState.slides.forEach((slide, index) => {
        const bulletPoints = typeof slide.content === 'string' 
            ? formatContentToBulletPoints(slide.content)
            : slide.content;
        
        const firstBulletPoint = bulletPoints[0] || { title: 'No content' };
        ...
        previewHTML += `
            <div class="slide-item ${index === presentationState.currentSlideIndex ? 'active' : ''}">
                ...
                <div class="slide-info">
                    ...
                    <div class="bullet-points-preview">
                        ${bulletPoints.map(point => `
                            <div class="bullet-point-box">
                                <div class="bullet-point-title">${point.title}</div>
                                <div class="bullet-point-content">${point.content}</div>
                            </div>
                        `).join('')}
                    </div>
                </div>
