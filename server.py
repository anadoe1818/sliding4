...
# Template categories and their corresponding folders
TEMPLATE_CATEGORIES = {
    'list': 'list_templates',
    'flow': 'flow_templates',
    'data': 'data_templates'
...
def get_template_path(category, template_name):
    ...
    return os.path.join(TEMPLATES_FOLDER, TEMPLATE_CATEGORIES[category], f"{template_name}.pptx")

def create_base_templates():
    """Create base templates for each category if they don't exist."""
    # List template
    list_template = Presentation()
    slide = list_template.slides.add_slide(list_template.slide_layouts[0])
    title_shape = slide.shapes.title
    title_shape.text = "List Template"
    list_template.save(get_template_path('list', 'default'))

    # Flow template
    flow_template = Presentation()
    slide = flow_template.slides.add_slide(flow_template.slide_layouts[5])  # Blank layout
    title_shape = slide.shapes.add_shape(1, Inches(1), Inches(1), Inches(8), Inches(1))
    title_shape.text = "Flow Template"
    flow_template.save(get_template_path('flow', 'default'))

    # Data template
    data_template = Presentation()
    slide = data_template.slides.add_slide(data_template.slide_layouts[5])  # Blank layout
    title_shape = slide.shapes.add_shape(1, Inches(1), Inches(1), Inches(8), Inches(1))
    title_shape.text = "Data Template"
    data_template.save(get_template_path('data', 'default'))

# Create base templates if they don't exist
for category in TEMPLATE_CATEGORIES:
    template_path = get_template_path(category, 'default')
...
def get_templates():
    """Get available templates for each category."""
    templates = {}
    for category, folder in TEMPLATE_CATEGORIES.items():
        category_path = os.path.join(TEMPLATES_FOLDER, folder)
        if os.path.exists(category_path):
            templates[category] = [f.replace('.pptx', '') for f in os.listdir(category_path) if f.endswith('.pptx')]
...
def save_presentation():
    try:
        ...
        for slide_data in slides:
            # Get slide type and template
            slide_type = slide_data.get('type', 'list')
            template_name = slide_data.get('template', 'default')
            
            # Load template for this slide type
            template_path = get_template_path(slide_type, template_name)
            if not os.path.exists(template_path):
                template_path = get_template_path(slide_type, 'default')
            
            template_prs = Presentation(template_path)
            # Use blank layout (layout 6) instead of title layout
            slide_layout = template_prs.slide_layouts[6]
            ...
            if slide_type == 'list':
                ...
                create_flow_chart(slide, content)
            elif slide_type == 'data':
                create_data_visualization(slide, content)
...
def create_flow_chart(slide, content):
    """Create a flow chart from the content."""
    # Implementation for flow chart creation
    pass

def create_data_visualization(slide, content):
    """Create data visualization from the content."""
